...
**Domain:** Static, GitHub-Pages-deployable, visually rich (psychedelic/cosmic) Spanish marketing site (multi-page) with evolution path to ecommerce + podcast + content
**Researched:** 2026-05-21
**Confidence:** HIGH (core stack), MEDIUM (animation library trade-off), HIGH (form backend), HIGH (hosting/deploy path)

---

## TL;DR Recommendation

**Use Astro 6.3.x + Tailwind CSS v4.3.x + vanilla CSS gradients/`@property` animations + GSAP 3.15.x (loaded only where needed) + Web3Forms (form fallback) + Calendly inline embed + Phosphor Icons.**
...
**Time-to-deploy on GH Pages:** Astro 30–60 min for first MVP commit (most of that is content, not config). Vanilla HTML+CSS is technically faster to *start* but slower to *evolve* into the post-MVP phases — see comparison table.
...
npm create astro@latest planetapsilo -- --template minimal --typescript strict --no-install --git no
...
npm install -D @astrojs/sitemap@^3.7.2
...
**`astro.config.mjs` for `Elsolarcg/planetapsilo` (GH Pages project repo):**
...
export default defineConfig({
  ...
  base: '/planetapsilo',           // CRITICAL: matches repo name, leading slash, no trailing slash
  ...
  build:
    ...
    steps:
      ...
      - uses: withastro/action@v3
        # Reads astro.config.mjs; uses Node 22 automatically
  deploy:
    needs: build
    runs-on: ubuntu-latest
...
Time-to-MVP = time from `git init` to a live URL on `https://elsolarcg.github.io/planetapsilo/` with Home + 3 subpages + Calendly + WhatsApp + form fallback + psychedelic styling.
...
**Verdict:** Astro wins on *every dimension except raw time-to-first-pixel against vanilla HTML*. The 15-min penalty vs. vanilla is recovered the moment you add the 5th page, the podcast feed, or want to share a `<Layout>` component.

---

## Visual Stack: How to Achieve "Otherworldly / Cosmic / Psychedelic"

Hierarchy of effort — **start at level 1, only escalate if MVP time allows**.
...
This alone gets ~80% of the reference site's aesthetic. **Total added bundle: 0 KB JS.**
...
<form action="https://api.web3forms.com/submit" method="POST">
  ...
  <input type="hidden" name="subject" value="Nuevo contacto planetapsilo">
  <input type="hidden" name="from_name" value="planetapsilo.com">
  <!-- honeypot -->
...
**If MVP must ship in <60 minutes (today's hard constraint):**
...
**If the 1–2h budget stretches to 3h:**
...
**If we later add ecommerce (phase 2 of art shop):**
...
**If we later add the podcast (phase 3):**
...
**If we later add bilingual ES/EN (phase 3+):**
...
*Stack research for: static, GH-Pages-deployable, visually rich Spanish marketing site (planetapsilo)*
*Researched: 2026-05-21*
